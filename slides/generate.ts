...
// movement\` after editing it.`,
  ...
  },  ...sweepSlides(),
  {
    ...
    body: `${bullets([
      ...
      "Thinking tokens range from 0 (the Claude tiers, the small models) to 145k (<code>seed-2.0-mini</code>, 47/48) for the same sixteen player turns",
    ])}`,
    notes: `All from the two results files for the sweep. One run per model,
    so any single lost check is as consistent with sampling as with a
    difference between models; the pattern across thirteen models losing in
    intake is the finding, not any one row.
    <br><br>The <code>&lt;code&gt;</code> tag is a parser gap found by this
    sweep and is filed in TODO.md.`,
  },
